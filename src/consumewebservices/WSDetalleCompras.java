    ...
    public DetalleCompraBean ejecutaWebService(String... params) {
        ...
        switch (params[1]) { 
            case "1" : 
                ...
                cantidad = params[6];
                descuento = params[7];
                unidadMedida = params[8];
                idSucursal = params[9];                   
                detalleCompraObj = guardaDetalleCompraWS(); break;
            case "2" : 
                idCompra = params[2];
                detalleCompraObj = eliminaDetalleCompraPorIdCompraWS(); break;
        }
...
//    }
    ...
    public DetalleCompraBean eliminaDetalleCompraPorIdCompraWS(String... params) {
        DetalleCompraBean elimina = null;
        try {
            HttpURLConnection urlConn;
            DataOutputStream printout;
            DataInputStream input;
           ...
           OutputStream os = urlConn.getOutputStream();
            ...
            if (respuesta == HttpURLConnection.HTTP_OK) {
                ...
                //Accedemos al vector de resultados
                int resultJSON = respuestaJSON.getInt("estado");   // estado es el nombre del campo en el JSON
                if (resultJSON == 1) {      // hay un detalleventa que mostrar
                    elimina = new DetalleCompraBean();
                } else if (resultJSON == 2) {
