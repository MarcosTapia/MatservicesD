...
public class WSClientesList {
    ...
    String devuelve ="";
        ...
        public ArrayList<ClienteBean> ejecutaWebService(String... params) {
            ...
            switch (params[1]) { 
                ...
                case "2" : clientes = buscaClienteIdWS(cadena); break;
                case "3" : clientes = busquedaClientesPorIdWS(cadena); break;
                case "4" : clientes = busquedaClientesPorNombreWS(cadena); break;
            }
            return clientes;
        }
    ...
    public ArrayList<ClienteBean> buscaClienteIdWS(String rutaWS) {
        ArrayList<ClienteBean> clientes = new ArrayList();
        try {
            url = new URL(cadena);
            HttpURLConnection connection = (HttpURLConnection) url.openConnection(); //Abrir la conexión
            ...
            if (respuesta == HttpURLConnection.HTTP_OK){
                ...
                if (resultJSON == 1) {
                    ClienteBean cli = new ClienteBean();
                    cli.setIdCliente(respuestaJSON.getJSONObject("cliente").getInt("idCliente"));
                    cli.setApellidos(respuestaJSON.getJSONObject("cliente").getString("apellidos"));
                    cli.setCiudad(respuestaJSON.getJSONObject("cliente").getString("ciudad"));
                    cli.setComentarios(respuestaJSON.getJSONObject("cliente").getString("comentarios"));
                    cli.setCp(respuestaJSON.getJSONObject("cliente").getString("cp"));
                    cli.setDireccion1(respuestaJSON.getJSONObject("cliente").getString("direccion1"));
                    cli.setDireccion2(respuestaJSON.getJSONObject("cliente").getString("direccion2"));
                    cli.setEmail(respuestaJSON.getJSONObject("cliente").getString("email"));
                    cli.setEmpresa(respuestaJSON.getJSONObject("cliente").getString("empresa"));
                    cli.setEstado(respuestaJSON.getJSONObject("cliente").getString("estado"));
                    cli.setNoCuenta(respuestaJSON.getJSONObject("cliente").getString("noCuenta"));
                    cli.setNombre(respuestaJSON.getJSONObject("cliente").getString("nombre"));
                    cli.setPais(respuestaJSON.getJSONObject("cliente").getString("pais"));
                    cli.setRfc(respuestaJSON.getJSONObject("cliente").getString("rfc"));
                    cli.setTelefono_casa(respuestaJSON.getJSONObject("cliente").getString("telefono_casa"));
                    cli.setTelefono_celular(respuestaJSON.getJSONObject("cliente").getString("telefono_celular"));
                    clientes.add(cli);
                }
            }
        } catch (MalformedURLException e) {
    ...
    public ArrayList<ClienteBean> busquedaClientesPorIdWS(String rutaWS) {
        ArrayList<ClienteBean> clientes = new ArrayList();
        try {
            url = new URL(cadena);
            HttpURLConnection connection = (HttpURLConnection) url.openConnection(); //Abrir la conexión
            ...
            if (respuesta == HttpURLConnection.HTTP_OK){
                ...
                int resultJSON = respuestaJSON.getInt("estado");
                if (resultJSON == 1) {
                    JSONArray clientesJSON = respuestaJSON.getJSONArray("cliente");   // estado es el nombre del campo en el JSON
                    for(int i=0;i<clientesJSON.length();i++){
                        ClienteBean cli = new ClienteBean();
                        cli.setApellidos(clientesJSON.getJSONObject(i).getString("apellidos"));
                        cli.setCiudad(clientesJSON.getJSONObject(i).getString("ciudad"));
    ...
    public ArrayList<ClienteBean> busquedaClientesPorNombreWS(String rutaWS) {
        ArrayList<ClienteBean> clientes = new ArrayList();
        try {
            url = new URL(cadena);
            HttpURLConnection connection = (HttpURLConnection) url.openConnection(); //Abrir la conexión
