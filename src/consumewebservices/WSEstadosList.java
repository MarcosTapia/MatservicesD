...
public class WSEstadosList {
    String cadena;
    URL url = null; // Url de donde queremos obtener información
    String devuelve ="";

        public ArrayList<EstadoBean> ejecutaWebService(String... params) {
            cadena = params[0];
            url = null; // Url de donde queremos obtener información
            devuelve ="";
            ArrayList<EstadoBean> estados = new ArrayList();
            switch (params[1]) { 
                case "1" : estados = obtenerEstadosWS(); break;
            }
            return estados;
        }
    ...
    public ArrayList<EstadoBean> obtenerEstadosWS() {
        ...
        try {
            ...
            if (respuesta == HttpURLConnection.HTTP_OK){
                ...
                if (resultJSON == 1) {
                    JSONArray estadosJSON = respuestaJSON.getJSONArray("estados");   // estado es el nombre del campo en el JSON
                    for(int i=0;i<estadosJSON.length();i++){
                        EstadoBean edo = new EstadoBean();
                        edo.setIdEstado(estadosJSON.getJSONObject(i).getInt("id"));
                        edo.setEstado(estadosJSON.getJSONObject(i).getString("estado"));
                        estados.add(edo);
                    }
                }
            }
