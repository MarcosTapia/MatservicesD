...
public class WSDetalleVentasList {
    String cadena;
    URL url = null; // Url de donde queremos obtener información
    String devuelve ="";

        public ArrayList<DetalleVentaBean> ejecutaWebService(String... params) {
            cadena = params[0];
            url = null; // Url de donde queremos obtener información
            devuelve ="";
            ArrayList<DetalleVentaBean> detalleVentas = new ArrayList();
            switch (params[1]) { 
                case "1" : detalleVentas = obtenerDetalleVentasWS(); break;
            }
...
//        }
    ...
    public ArrayList<DetalleVentaBean> obtenerDetalleVentasWS() {
        ArrayList<DetalleVentaBean> detalleVentas = new ArrayList();
        try {
            url = new URL(cadena);
            ...
            if (respuesta == HttpURLConnection.HTTP_OK){
                ...
                if (resultJSON == 1) {
                    JSONArray detalleVentasJSON = respuestaJSON.getJSONArray("detalleVentas");   // estado es el nombre del campo en el JSON
                    for(int i=0;i<detalleVentasJSON.length();i++){
                        DetalleVentaBean detalleVenta = new DetalleVentaBean();
                        detalleVenta.setIdDetalleVenta(detalleVentasJSON.getJSONObject(i).getInt("idDetalleVenta"));
                        detalleVenta.setIdVenta(detalleVentasJSON.getJSONObject(i).getInt("idVenta"));
                        detalleVenta.setIdArticulo(detalleVentasJSON.getJSONObject(i).getInt("idArticulo"));
                        detalleVenta.setPrecio(detalleVentasJSON.getJSONObject(i).getDouble("precio"));
                        detalleVenta.setCantidad(detalleVentasJSON.getJSONObject(i).getDouble("cantidad"));
                        detalleVenta.setDescuento(detalleVentasJSON.getJSONObject(i).getDouble("descuento"));
                        detalleVenta.setUnidadMedida(detalleVentasJSON.getJSONObject(i).getString("unidadMedida"));
                        detalleVenta.setIdSucursal(detalleVentasJSON.getJSONObject(i).getInt("idSucursal"));
